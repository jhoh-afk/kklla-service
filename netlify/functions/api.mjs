...
export default async function handler(req) {
  ...
  try {
    ...
    if (method === "POST" && path === "/api/login") return handleLogin(req, sql);
    if (method === "POST" && path === "/api/logout") return handleLogout(req, sql);
    if (method === "POST" && path === "/api/signup") return handleSignup(req, sql);

    const user = await requireUser(req, sql);
    if (method === "GET" && path === "/api/me") return json({ user: serializeUser(user) });
    if (method === "GET" && path === "/api/bootstrap") return handleBootstrap(req, sql, user);
    if (method === "GET" && path === "/api/settlements") return handleSettlements(req, sql, user, url);
    if (method === "POST" && path === "/api/accounts") return handleCreateAccount(req, sql, user);
    if (method === "POST" && path === "/api/activities") return handleCreateActivity(req, sql, user);
    if (method === "POST" && path === "/api/contracts") return handleCreateContract(req, sql, user);

    const userStatusMatch = path.match(/^\/api\/users\/([^/]+)\/status$/);
    if (method === "PATCH" && userStatusMatch) return handleUpdateUserStatus(req, sql, user, userStatusMatch[1]);
    const accountMatch = path.match(/^\/api\/accounts\/([^/]+)$/);
    if (method === "PATCH" && accountMatch) return handleUpdateAccount(req, sql, user, accountMatch[1]);
    const approveMatch = path.match(/^\/api\/contracts\/([^/]+)\/approve$/);
    if (method === "PATCH" && approveMatch) return handleApproveContract(req, sql, user, approveMatch[1]);
